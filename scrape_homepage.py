#!/usr/bin/env python3
"""
Scrape the homepage from mrpfx.com and save all assets locally.
Similar to how the mentorship page was scraped.
"""
...
def main():
    print(f"Fetching homepage from {BASE_URL}...")
    ...
    # Save raw HTML first
    with open(os.path.join(OUTPUT_DIR, 'homepage_raw.html'), 'w', encoding='utf-8') as f:
        f.write(response.text)
    print(f"Saved raw HTML to {OUTPUT_DIR}/homepage_raw.html")
    ...
    for url in image_urls:
        if 'mrpfx.com' in url and '/wp-content/' in url:
            ...
            if download_file(url, local_path):
                ...
                image_map[url] = f"/assets/home/{filename}"
    ...
    css_to_download = [
        ("https://mrpfx.com/wp-content/plugins/elementor/assets/css/frontend.min.css", "homepage-elementor.css"),
        ("https://mrpfx.com/wp-content/themes/developer/style.css", "homepage-theme.css"),
    ]
    ...
    if main_content:
        ...
        # Also replace partial URLs
        for original_url, local_path in image_map.items():
            # Handle srcset with width descriptors
            filename = os.path.basename(urlparse(original_url).path)
            if filename in html_content:
                # Replace just the filename part in srcsets
                pass  # Keep the mapping for now

        # Save cleaned HTML
        cleaned_path = os.path.join(OUTPUT_DIR, 'homepage_cleaned.html')
...
'''
        component_path = 'components/home/ScrapedHomePage.tsx'
    ...
    print(f"   - Component saved to: components/home/ScrapedHomePage.tsx")
