#!/usr/bin/env python3
"""
Scrape the VIP Signals Group page from mrpfx.com and save all assets locally.
Similar to how the homepage and mentorship pages were scraped.
"""
...
# Configuration
BASE_URL = "https://mrpfx.com/vip-signals-group/"
OUTPUT_DIR = "scraped_assets"
IMAGES_DIR = "public/assets/vip"
CSS_DIR = "app"
...
def main():
    print(f"Fetching VIP Signals Group page from {BASE_URL}...")
    ...
    # Save raw HTML first
    with open(os.path.join(OUTPUT_DIR, 'vip_raw.html'), 'w', encoding='utf-8') as f:
        f.write(response.text)
    print(f"Saved raw HTML to {OUTPUT_DIR}/vip_raw.html")
    ...
    for url in image_urls:
        if 'mrpfx.com' in url and '/wp-content/' in url:
            ...
            if download_file(url, local_path):
                ...
                image_map[url] = f"/assets/vip/{filename}"
    ...
    if main_content:
        ...
        # Save cleaned HTML
        cleaned_path = os.path.join(OUTPUT_DIR, 'vip_cleaned.html')
...
'''
        component_path = 'components/vip/ScrapedVIPPage.tsx'
        os.makedirs(os.path.dirname(component_path), exist_ok=True)
        with open(component_path, 'w', encoding='utf-8') as f:
            f.write(component_content)
        print(f"Generated React component: {component_path}")

    print("\n✅ VIP Signals Group page scraping complete!")
    print(f"   - Images saved to: {IMAGES_DIR}/")
    print(f"   - HTML saved to: {OUTPUT_DIR}/")
    print(f"   - Component saved to: components/vip/ScrapedVIPPage.tsx")
