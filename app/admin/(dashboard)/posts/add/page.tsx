...
import { WPMediaItem, adminTermService } from '@/lib/admin-api';
...
export default function AddPostPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (featuredImage) {
                await postsService.updateFeaturedImage(newPost.ID, featuredImage.id);
            }

            // Assign Terms
            const termIds = [...selectedCategories, ...selectedTags];
            if (termIds.length > 0) {
                await adminTermService.assignTerms(newPost.ID, termIds);
            }

            setCreatedId(newPost.ID);
            setSuccessModal(true);
        } catch (error) {
            console.error('Failed to create post:', error);
            setErrorModal({ isOpen: true, message: 'Failed to create post. Please try again.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">Write and publish a new blog post</p>
                    </div>
                </div>
                <button
            ...
            <SuccessModal
                ...
                onConfirm={() => router.push(`/admin/posts/${createdId}`)}
